...
const Links = ({links, linkRemove, linkToRemove, linkList, setLinkToRemove}) => {
    ...
    const confirmRemove = (e) => (linkToRemove ? linkRemove(linkToRemove) : null)
        ...
        <Layout>
                ...
                {linkToRemove ? (
                    <div className="alert alert-danger rounded float-center shadow-bold">
                        <h4 className="alert-heading">Delete Confimation!</h4>
                        <p>Are you sure you want to delete, this action cannot be undone</p>
                        <hr/>
                        <div className="d-flex justify-content-between">
                            <button className="btn btn-outline-light" onClick={cancelDelete}>Cancelar</button>
                            <button className="btn btn-denger" onClick={confirmRemove}>Apagar</button>
                        </div>
                    </div>
                ) : null}
