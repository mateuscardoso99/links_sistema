...
const saltRounds = 10//os metodos de criptografia serao rodados 10 vezes
...
router.post('/sign-in', accountSignIn, async (req, res)=>{
    ...
    //validar a senha
    const match = account ? bcrypt.compareSync(password, account.password) : null //verificando se as senhas são iguais
...
router.post('/sign-up', accountSignUp, async (req, res)=>{
    ...
    const account = await Account.findOne({where: { email }})//verificando se email ja existe
    if(account) return res.jsonBadRequest(null, getMessage('account.signup.email_exists'))

    //const salt = 'irue92@e' -> caracteres que serao incluidos junto com a criptografia
    const hash = bcrypt.hashSync(password, saltRounds)//criptografando senha
